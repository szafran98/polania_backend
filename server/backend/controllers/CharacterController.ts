...
export default class CharacterController {
    static listAllUserCharacters = async (req: Request, res: Response) => {
        ...
        try {
            ...
            await userRepository
                .findOneOrFail(id)
                .then(async (res) => {
                    userCharacters = await characterRepository.findByIds(
                        res.charactersIds
                    );
    ...
    static createNewCharacter = async (req: Request, res: Response) => {
        ...
        // console.log;
        middlePayload = Buffer.from(middlePayload, 'base64').toString('ascii');

        const userId = JSON.parse(middlePayload).userId;
        const characterName = req.body.name;
        const selectedClass = req.body.class;
        console.log(selectedClass);
        console.log('seected class ^^^^');
