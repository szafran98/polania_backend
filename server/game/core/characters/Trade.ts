...
export default class Trade {
    ...
    constructor (player1: Player, player2: Player) {
      this.player1 = {
        instance: game.PLAYERS_LIST.find(
    ...
    playerOfferListener () {
      this.player1.instance.playerSocket.on(
        ...
        (itemData: IOwnedItem) => {
          ...
          const itemOfferedByPlayer = this.player1.instance.statistics.equipment.backpack.find(
      ...
      this.player2.instance.playerSocket.on(
        ...
        (itemData: IOwnedItem) => {
          if (this.player1 === null || this.player2 === null) return
          console.log(this)
          const itemOfferedByPlayer = this.player2.instance.statistics.equipment.backpack.find(
            (item) => item.itemData.id === itemData.itemData.id
          )
    ...
    async isTradeAcceptedByParticipants () {
      ...
      if (this.player1.isOfferAccepted && this.player2.isOfferAccepted) {
        try {
          // GET PLAYER1 ITEM
          const player1Item = this.player1.instance.statistics.equipment.backpack.find(
            (itemInstance) => itemInstance.id === this.player1.offer.id
          )

          // DELETE ITEM IN BACKPACK
          const player1ItemIndex = this.player1.instance.statistics.equipment.backpack.indexOf(
            player1Item
          )
          ...
          // GET PLAYER2 ITEM
          const player2Item = this.player2.instance.statistics.equipment.backpack.find(
            (itemInstance) => itemInstance.id === this.player2.offer.id
          )

          // DELETE ITEM IN BACKPACK
          const player2ItemIndex = this.player2.instance.statistics.equipment.backpack.indexOf(
            player2Item
          )
