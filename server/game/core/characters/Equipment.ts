import { IOwnedItem, IStats } from '../../../../Interfaces'
...
export class Equipment {
    ...
    static getFirstEmptyFieldIdInBackpack (player: Player): string {
      ...
      for (let i = 1; i < 42; i++) {
        const playerBackpack = player.statistics.equipment.backpack
        const instanceInCheckedField = playerBackpack.find(
          (itemInField) => itemInField.fieldInEquipment === `field${i}`
        )
      ...
      if (
        ...
        for (const property of eqProperties) {
          if (property === afterDrag.itemData.type) {
            ...
            await manager.updateOne(
              ...
              {
                $set: {
                  fieldInEquipment: afterDrag.fieldInEquipment
                }
              }
            )
          }
        ...
        for (const property of eqProperties) {
          ...
          if (property === afterDrag.fieldInEquipment) {
            ...
            const indexInBackpack = this.backpack.indexOf(beforeDrag)
        ...
        console.log('case 3')
        for (const item in this.backpack) {
          if (
