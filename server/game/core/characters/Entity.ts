...
import { ICollisionEntity, IEntity } from '../../../../Interfaces'
import { game } from '../../../../app'
import { IGridObject } from '../Fight'
...
export default class Entity implements IEntity {
    ...
    attack (enemy: Entity): number {
      let trueDamage
      const generatedHit = this.statistics.generateHit
    ...
    moveForward (): boolean {
      ...
      game.ACTUAL_COMBATS_LIST.forEach((combat) => {
        for (let i = 0; i < combat.actualGridPresentation.length; i++) {
          combat.actualGridPresentation[i].forEach(
            (field: IGridObject) => {
              if (
                Object.prototype.hasOwnProperty.call(field, [
                  'entity'
                ]) &&
                            field.entity.id === this.id &&
                            !isMoveDone
              ) {
                const index = combat.actualGridPresentation[
                  i
                ].indexOf(field)

                const gridObject = field
                ...
                if (i === 0) {
                  if (
                    ...
                    // @ts-ignore
                    combat.actualGridPresentation[i][
                      index
                    ] = {}
